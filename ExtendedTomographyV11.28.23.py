...
class TomographyClass:
    ...
    def Submit(self):
        ...
        if self.Max == 360.0 and self.Start == 0.0 and self.Few == 0:
            #print('We are in where we want for the 0.6 test\n')
            self.DegreeList = np.arange(self.Start, self.Max, self.Rotation).tolist()
        else:
        ...
        if self.Max > 180.0 and self.Few == 0:
            self.HighMax = True
            self.HighMaxList = np.arange(self.Start, (self.Max),self.Rotation).tolist()
    ...
    def CheckDivisibility(self,StartValue,MaximumValue,Rotation):
        ...
        if round(((MaximumValue-StartValue) % Rotation),8) == 0.0:
            self.Proceed1 = 1
            print(self.Proceed1)
            #self.waitVar1.set(1)
        else:
