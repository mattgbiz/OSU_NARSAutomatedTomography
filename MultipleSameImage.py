...
import time, pytz, subprocess, os.path
...
class TomographyClass:
    def __init__(self,root):
        ...
        self.MMReadyButton.grid(row=1,column=1,columnspan=2)
        ...
        self.SMCResultLabel.grid(row=6,column=1,columnspan=2)
    ...
    def ChangeStart(self):
        ...
        self.Start = self.ChangeStartEntry.get()
    ...
    def ChangeMax(self):
        ...
        self.Max = self.ChangeMaxEntry.get()
    ...
    def ChangeRotation(self):
        ...
        self.Rotation = self.ChangeRotationEntry.get()
    ...
    def StartExperiment(self):
        ...
        self.Log.write('Input Parameters- Exposure Time: {} Degree Increment: {} Start Degree: {} Max Degree: {} \r\n'.format(self.ExposureTime,self.Rotation,self.Start,self.Max))
    ...
    def ControlPeripherals(self):
        ...
        for deg in self.DegreeList:
            ...
            try:
                #self.SMC.improved_move_abs(deg)
                print('would move')
            except:
            ...
            self.NewGenMessage = 'There are {} images left it has been {} minutes since start'.format(self.ImagesLeft,self.TimeElapsed)

        #after we leave the for loops tell user and kill the threads
        self.GeneralMessage('Tomography has completed')
    ...
    def ShowXYZ(self):
        ...
        self.XMoveLabel = Label(self.ControlXYZ,text='X Movements: Range +/-62.3875mm (2.4562in)')
        self.XZeroButton = Button(self.ControlXYZ,text='X Zero',command=lambda: self.ZeroStage('X'))
        self.YMoveLabel = Label(self.ControlXYZ,text='Y Movements: Range +/-62.570mm (2.4634in)')
        self.YZeroButton = Button(self.ControlXYZ,text='Y Zero',command=lambda: self.ZeroStage('Y'))
        self.ZMoveLabel = Label(self.ControlXYZ,text='Z Movements: Range +/-55.00mm (1.7433in)')
    ...
    def ConnectXYZ(self):
        ...
        self.XYZUsed = True
        if self.LimitZVar.get() == 0:
            #Change the limit to what it was before
            Zlimit = (44.28/0.0025)
            self.ZMoveLabel.grid_forget()
            self.ZMoveLabel = Label(self.ControlXYZ,text='Z Movements: Range +/-44.28mm (1.7433in)')
            self.ZMoveLabel.grid(row=0,column=8,columnspan=3)
        else:
            #If the stage is not blocked, the Zlimit Remains as is
            Zlimit = 22050
        try:
            self.XYZ = XYZMotion_Station.VXM(self.XYZPortEntry.get(),9600,0.1,Zlimit)
        except:
    ...
    def ZeroStage(self,Stage):
        if self.XYZConnected == True:
            ...
            self.XYZGoToExperimental.grid_forget()
            self.XYZGoToExperimental = Button(self.InitiateXYZ,text='Go To Experimental Positions',padx=2,pady=5,command=self.ExperimentalPositions)
            self.XYZGoToExperimental.grid(row=2,column=1)
        else:
            self.GeneralMessage('XYZ Stage was not connected, please connect before moving')
