#This file is for running tomography with Manual XYZ Stage control
#difference from normal tomography is the need for multiple day runs and how many images taken per run
from tkinter import *
import XYZMotion_Station, SMC100Final, NewCameraControl#FewViewPart2 CameraControl
import time, pytz, subprocess, os.path, shutil,os,sys 
...
class TomographyClass:
    def __init__(self,root):
        ...
        self.XYZUsed = False
        #same for the closed variables true
        self.SMCClose = True
        self.CameraClose = True
        self.XYZClose = True
        self.ExceptionThrown = False
        self.ExceptionIncrement = 0
        self.HighMax = False
        self.YZMove = -24.5
        ...
        self.FileNameEntry = Entry(self.InputTomographyFrame,width=68)
        self.FileNameEntry.insert(0,'Experiment')
        self.FileLocationLabel = Label(self.InputTomographyFrame, text= 'File Location')
        self.FileLocationEntry = Entry(self.InputTomographyFrame, width = 68)
        ...
        self.NumberDaysLabel = Label(self.InputTomographyFrame, text='Days for Experiment')
        self.NumberDaysEntry = Entry(self.InputTomographyFrame)
        self.NumberDaysEntry.insert(0,'3')
        ...
        self.RotationLabel = Label(self.InputTomographyFrame, text='Rotation (deg)')
        self.RotationEntry = Entry(self.InputTomographyFrame)
        self.RotationEntry.insert(0,'1.0')
        ...
        self.StartDegreeLabel = Label(self.InputTomographyFrame, text='Start Point (deg)')
        self.StartDegreeEntry = Entry(self.InputTomographyFrame)
        self.StartDegreeEntry.insert(0,'0.0')
        ...
        self.MaxDegreeLabel = Label(self.InputTomographyFrame, text='Max Rotation(deg)')
        self.MaxDegreeEntry = Entry(self.InputTomographyFrame)
        self.MaxDegreeEntry.insert(0,'360.0')
        self.FewVar = IntVar()
        self.FewVar.set(0)
        self.FewViewCheck = Checkbutton(self.InputTomographyFrame,text='Select if Running Few View', variable=self.FewVar)
        self.SubmitButton = Button(self.InputTomographyFrame,text='Enter',command=self.Submit)
        #for the size of object
        self.Y1Label = Label(self.InputTomographyFrame, text='First Y Pos (mm)')
        self.Y1Entry = Entry(self.InputTomographyFrame)
        #self.HeightEntry.insert(0,'60.0')
        self.Y2Label = Label(self.InputTomographyFrame, text='Second Y Pos (mm)')
        self.Y2Entry = Entry(self.InputTomographyFrame)
        self.Z1Label = Label(self.InputTomographyFrame, text='First Z Pos (mm)')
        self.Z1Entry = Entry(self.InputTomographyFrame)
        #self.HeightEntry.insert(0,'60.0')
        self.Z2Label = Label(self.InputTomographyFrame, text='Second Z Pos (mm)')
        self.Z2Entry = Entry(self.InputTomographyFrame)
        #self.WidthEntry.insert(0,'60.0')
        #self.HeightLabel = Label(self.InputTomographyFrame, text='Object Height(mm)')
        #self.HeightEntry = Entry(self.InputTomographyFrame)
        #self.HeightEntry.insert(0,'60.0')
        #self.WidthLabel = Label(self.InputTomographyFrame, text='Object Width(mm)')
        #self.WidthEntry = Entry(self.InputTomographyFrame)
        #self.WidthEntry.insert(0,'60.0')
        ...
        self.NumberDaysLabel.grid(row=3,column=2)
        self.NumberDaysEntry.grid(row=3,column=3)
        self.RotationLabel.grid(row=4,column=0)
        self.RotationEntry.grid(row=4,column=1)
        self.SMCPortLabel.grid(row=4,column=2)
        self.SMCPortEntry.grid(row=4,column=3)
        self.StartDegreeLabel.grid(row=5,column=0)
        self.StartDegreeEntry.grid(row=5,column=1)
        self.CameraPortLabel.grid(row=5,column=2)
        self.CameraPortEntry.grid(row=5,column=3)
        self.MaxDegreeLabel.grid(row=6,column=0)
        self.MaxDegreeEntry.grid(row=6,column=1)
        self.FewViewCheck.grid(row=6,column=2)
        self.SubmitButton.grid(row=6,column=3)
        #large object items
        #self.HeightLabel.grid(row=7,column=0)
        #self.HeightEntry.grid(row=7,column=1)
        #self.WidthLabel.grid(row=7,column=2)
        #self.WidthEntry.grid(row=7,column=3)
        self.Y1Label.grid(row=7,column=0)
        self.Y1Entry.grid(row=7,column=1)
        self.Y2Label.grid(row=7,column=2)
        self.Y2Entry.grid(row=7,column=3)
        self.Z1Label.grid(row=8,column=0)
        self.Z1Entry.grid(row=8,column=1)
        self.Z2Label.grid(row=8,column=2)
        self.Z2Entry.grid(row=8,column=3)
        #************************************************************************************
        #Initialize Programs Frame
        self.MMOverallLabel = Label(self.InitializeTomographyFrame, text='If MicroManager is not open, close this program and open it. Restart and hit "MM Ready"')
        ...
        self.SnapExposLabel = Label(self.InitializeTomographyFrame, text='Snap Exposure (s):')
        self.SnapExposEntry = Entry(self.InitializeTomographyFrame,width=5)
        self.SnapExposEntry.insert(0,'30.0')
        self.SnapEMGainLabel = Label(self.InitializeTomographyFrame,text='Snap EM Gain:')
        self.SnapEMGainEntry = Entry(self.InitializeTomographyFrame,width=5)
        self.SnapEMGainEntry.insert(0,'3')
        self.SnapImageButton = Button(self.InitializeTomographyFrame,text='Snap Image',command= self.SnapImage)
        self.SnapImageLabel = Label(self.InitializeTomographyFrame,text='Snap Name:')
        self.SnapImageEntry = Entry(self.InitializeTomographyFrame,width=15)
        self.SnapImageEntry.insert(0,'TestSnap_1')
        ...
        self.MMOverallLabel.grid(row=0,column=0,columnspan=4)
        self.MMReadyButton.grid(row=1,column=3,sticky='e')#,columnspan=2)
        self.CameraOverallLabel.grid(row=2,column=0,columnspan=4)
        self.SnapExposLabel.grid(row=3,column=0,sticky='w')
        self.SnapExposEntry.grid(row=3,column=1,sticky='w')
        self.SnapEMGainLabel.grid(row=3,column=2,sticky='w')
        self.SnapEMGainEntry.grid(row=3,column=3,sticky='w')
        self.SnapImageButton.grid(row=4,column=2,sticky='e')
        self.SnapImageLabel.grid(row=4,column=0,sticky='w')
        self.SnapImageEntry.grid(row=4,column=1,sticky='w')
        self.CameraReadyButton.grid(row=4,column=3,sticky='e')
        self.SMCOverallLabel.grid(row=5,column=0,columnspan=4)
        self.TestSMCButton.grid(row=6,column=2,sticky='e')
        self.SMCResultLabel.grid(row=6,column=0,columnspan=2)
        self.SMCReadyButton.grid(row=6,column=3,sticky='e')
        ...
        PNGShown = True
        #toss these on the screen
        self.ContrastLabel.grid(row=0,column=0)
        self.ContrastEntry.grid(row=0,column=1)
        self.ImageLabel2.grid(row=1,column=0,columnspan=2)
        #************************************************************************************
        #General/Start Frame
        self.StartButton = Button(self.GeneralFrame,text='Start',bg='green',fg='white',padx=20,pady=10,command=self.StartExperiment)
        self.StartButton['state'] = DISABLED
        #self.PauseButton = Button(self.GeneralFrame,text='Pause',bg='yellow',fg='black',padx=20,pady=10,command=self.Pause)
        #self.PauseButton['state'] = DISABLED
        #self.Paused = False
        self.SendRemainingButton = Button(self.GeneralFrame,text='Remaining To Text',wraplength=60,padx=5,pady=3,command=self.RemainingToText)
        self.CloseButton = Button(self.GeneralFrame,text='Quit',bg='red',wraplength=30,padx=15,pady=10,command=self.Stop)
        ...
        self.SendRemainingButton.grid(row=0,rowspan=3,column=1,padx=5)
        self.CloseButton.grid(row=0,rowspan=3,column=2,padx=5)
        self.GeneralMessageLabel.grid(row=0,column=3,padx=5)
        self.SaveMessageLabel.grid(row=1,column=3,padx=5)
        self.ErrorMessageLabel.grid(row=2,column=3,padx=5)
    ...
    def Submit(self):
        ...
        #new data the number of days and number of images per degree
        self.NumberPerDeg = int(self.NumberImagesEntry.get())
        self.NumberDays = int(self.NumberDaysEntry.get())
        #object height and width
        #self.ObjHeight = float(self.HeightEntry.get())
        #self.ObjWidth = float(self.WidthEntry.get())
        #now I will do a couple of checks to make sure we are actually doing tomography for a large item
        self.TotalMoves = 0
        try:
            self.Y1Pos = float(self.Y1Entry.get())
            self.Y2Pos = float(self.Y2Entry.get())
        except:
            print('There were either no entries or invalid entries for Y positions')
            print('It is assumed that no movements are necessary in Y direction so the object is tall not wide')
            self.HorizontalMoves = False
        else:
            self.HorizontalMoves = True
            self.TotalMoves = self.TotalMoves+2
        try:
            self.Z1Pos = float(self.Z1Entry.get())
            self.Z2Pos = float(self.Z2Entry.get())
        except:
            print('There were either no entries or invalid entries for Z positions')
            print('It is assumed that no movements are necessary in Z direction so the object is wide not tall')
            self.VerticalMoves = False
        else:
            self.VerticalMoves = True
            self.TotalMoves = self.TotalMoves+2
        ...
        self.PathCheck = self.FileLocation + '/' + self.FileName + str(self.Start) + '.tif'
        ...
        self.DegreeList = list(np.round(self.DegreeList,8))

        print('Here is the final degree list to be used: {}'.format(self.DegreeList))
        
        if self.Max > 180.0:
            self.HighMax = True
            #make the degree message list be the positive versions from 0-360 instead of -180-180
            self.DegMes = [x+180.0 for x in self.DegreeList]
            self.DegMes = list(np.round(self.DegMes,8))
        else:
            self.DegMes = self.DegreeList

        #remove the 360 if we are starting at 0 and ending with 360
        if self.Max == 360.0 and self.Start == 0.0:
            if self.Few == 1:
                for x in range(len(self.DegreeList)-1):    #since I will be removing something from the list the length needs to be -1
                    if self.DegreeList[x] == 180.0:
                        self.DegreeList.remove(180.0) 
                    if self.DegMes[x] == 360.0:
                        self.DegMes.remove(360.0)
            else:
                self.DegreeList.pop()
                self.DegMes.pop()

        #Now here we need to split the degree lists based on how many days will be run
        #for example if running 3 days 360 total degrees, it'd be 120 120 120 for each day
        #find number of degrees that would be done based on total length and number of days
        self.DegsPerDay = int(len(self.DegreeList)/self.NumberDays)                
        print('Number of Degrees per day is {}'.format(self.DegsPerDay))
        #Now get the chunked up lists
        self.ChunksList = [self.DegreeList[x:x+self.DegsPerDay] for x in range(0, len(self.DegreeList), self.DegsPerDay)]
        self.ChunksMes = [self.DegMes[x:x+self.DegsPerDay] for x in range(0,len(self.DegMes), self.DegsPerDay)]
        #Chunks should have the a number of sublists eqaul to the number of days
        #the first day aka run for this GUI today, will be Chunks[0]
        self.DegreeListDay1 = self.ChunksList[0].copy()
        self.DegMesDay1 = self.ChunksMes[0].copy()
        print('Degree List for the first day {}'.format(self.DegreeListDay1))
        #Now all of the degree lists need to be sent to their own textfiles
        for i in range(1,self.NumberDays+1):
            TextFileDegrees = self.FileLocation+'\DegreesForDay#'+str(i)+'.txt'
            TextFile = open(TextFileDegrees,'w+')
            np.savetxt(TextFile,self.ChunksList[(i-1)],fmt='%0.2f')
            TextFile.close()

        #When all done Return the optional frame to black
        self.OptionalFrame.grid_forget()
        self.OptionalFrame = LabelFrame(self.TomographyFrame,text='Remaining Degrees for First Day Listed Here',bg='black',fg='white',padx=10,pady=10)
        self.OptionalFrame.grid(row=1,column=2)
        self.HiddenLabel = Label(self.OptionalFrame,text='Houdini',bg='black',fg='black')
        self.HiddenLabel.grid(row=0,column=0)
        self.DegreeMessage = ' '.join(map(str, self.DegMesDay1))
        self.ListMessage(self.DegreeMessage)

    def CheckDivisibility(self,StartValue,MaximumValue,Rotation):
        print('About To print the remainder result')
        print((MaximumValue-StartValue)%Rotation)
        if (MaximumValue-StartValue) % Rotation == 0.0:
            self.Proceed1 = 1
            print(self.Proceed1)
            #self.waitVar1.set(1)
        else:
            self.OptionalFrame.grid_forget()
            self.OptionalFrame = LabelFrame(self.TomographyFrame, text = 'Divisibility Error Option')
            self.OptionalFrame.grid(row=1,column=2)
            #Make the Options on the frame
            self.InstructionLabel = Label(self.OptionalFrame, text='The Input Start/Max/Rotation degrees do not match')
            self.InstructionLabel2 = Label(self.OptionalFrame, text='Change one of the values so the rotations align')
            self.ChangeStartLabel = Label(self.OptionalFrame, text = 'Start:')
            self.ChangeStartEntry = Entry(self.OptionalFrame)
            self.ChangeStartEntry.insert(0,str(StartValue))
            self.ChangeMaxLabel = Label(self.OptionalFrame, text='Max:')
            self.ChangeMaxEntry = Entry(self.OptionalFrame)
            self.ChangeMaxEntry.insert(0,str(MaximumValue))
            self.ChangeRotationLabel = Label(self.OptionalFrame, text='Rotation:')
            self.ChangeRotationEntry = Entry(self.OptionalFrame)
            self.ChangeRotationEntry.insert(0,str(Rotation))
            self.ChangeStartButton = Button(self.OptionalFrame, text='Change Start',command=self.ChangeStart)
            self.ChangeMaxButton = Button(self.OptionalFrame,text='Change Max',command=self.ChangeMax)
            self.ChangeRotationButton = Button(self.OptionalFrame,text='Change Rotation',command=self.ChangeRotation)
            #Put them on the screen
            self.InstructionLabel.grid(row=0,column=0,columnspan=6)
            self.InstructionLabel2.grid(row=1,column=0,columnspan=6)
            self.ChangeStartLabel.grid(row=2,column=0)
            self.ChangeStartEntry.grid(row=2,column=1)
            self.ChangeMaxLabel.grid(row=2,column=2)
            self.ChangeMaxEntry.grid(row=2,column=3)
            self.ChangeRotationLabel.grid(row=2,column=4)
            self.ChangeRotationEntry.grid(row=2,column=5)
            self.ChangeStartButton.grid(row=3,column=0,columnspan=2)
            self.ChangeMaxButton.grid(row=3,column=2,columnspan=2)
            self.ChangeRotationButton.grid(row=3,column=4,columnspan=2)

    def ChangeStart(self):
        print('Changed the start')
        self.ErrorMessage('Changed the start')
        self.Start = float(self.ChangeStartEntry.get())
        self.StartDegreeEntry.delete(0,END)
        self.StartDegreeEntry.insert(0,str(self.Start))
        self.waitVar1.set(1)
        self.Proceed1 = 1
    def ChangeMax(self):
        print('Change max')
        self.ErrorMessage('Changed Max')
        self.Max = float(self.ChangeMaxEntry.get())
        self.MaxDegreeEntry.delete(0,END)
        self.MaxDegreeEntry.insert(0,str(self.Max))
        self.waitVar1.set(1)
        self.Proceed1 = 1 
    def ChangeRotation(self):
        print('Change Rotation')
        self.ErrorMessage('Changed Rotation Degree')
        self.Rotation = float(self.ChangeRotationEntry.get())
        self.RotationEntry.delete(0,END)
        self.RotationEntry.insert(0,str(self.Rotation))
        self.waitVar1.set(1)
        self.Proceed1 = 1
    ...
    #****************************************************************************
    #Functions that go with Opening Programs and initializing devices
    ...
    def MMReady(self):
        ...
        self.MMReadyButton.grid(row=1,column=3,sticky='e')
    ...
    def SnapImage(self):
        ...
        self.SnapExposure = float(self.SnapExposEntry.get())*1000
        self.SnapEMGain = int(self.SnapEMGainEntry.get())
        #Get the entry and split it between the _
        self.SnapGet = self.SnapImageEntry.get().split('_')
        self.SnapName = self.SnapGet[0]
        self.SnapIncrement = int(self.SnapGet[1])
        if os.path.exists(self.FileLocationEntry.get()) == False:
            os.mkdir(self.FileLocationEntry.get())
            self.ErrorMessage('Created a new folder as the input one did not exist')
        ...
        self.Image = self.Camera.TakeImage(self.SnapExposure,self.FileLocationEntry.get(),self.SnapName,self.SnapIncrement,self.ContrastVal,self.SnapEMGain)
        self.ShowContrast(self.FileLocationEntry.get()+'/ReSizedJPGs/'+self.SnapName+str(self.SnapIncrement)+'_contrast'+'.jpeg')
        #self.CameraIncrement +=1
        self.SnapIncrement +=1
        self.SnapImageEntry.delete(0,END)
        self.SnapImageEntry.insert(0,self.SnapName+'_'+str(self.SnapIncrement))
    ...
    def StartExperiment(self):
        ...
        self.Log.write('The Experiment Name is {} and is saving to {} \r\n'.format(self.FileName,self.FileLocation))
        ...
        if self.FirstPick == True: 
            ...
            self.Camera = NewCameraControl.CameraControl(self.CameraPort)
        #initialize the xyz stage if it hasn't been connected.
        if self.XYZConnected != True:
            self.ConnectXYZ()
        #Then we get the initial positions of the XYZ stage now
        #self.InitialYPos = -1*self.XYZ.getPosition('Y')
        #self.InitialZPos = -1*self.XYZ.getPosition('Z')
        #if self.InitialYPos != self.Y1Pos:
            #self.XYZ.abs_move_mm('Y',-1*self.Y1Pos)
        #print('Initial Y: {} and Z: {}'.format(self.InitialYPos,self.InitialZPos))
    ...
    def ControlPeripherals(self):
        ...
        #here we have the capability for 0, 2, or 4 moves and if it is zero moves then we are just doing normal tomography at the starting position
        while self.moveInc < self.TotalMoves:
            lastDegree = self.DegreeListDay1[0]
            self.RemainingList = self.DegreeListDay1
            #go to the first [0]th 
            if self.moveInc == 0 and self.TotalMoves == 4:
                self.XYZ.abs_move_mm('Y',-1*self.Y1Pos) #first go to the first y and z positions
                self.XYZ.abs_move_mm('Z',-1*self.Z1Pos)
            elif self.moveInc == 0 and self.TotalMoves == 2 and self.HorizontalMoves == True:
                self.XYZ.abs_move_mm('Y',-1*self.Y1Pos)
            elif self.moveInc == 0 and self.TotalMoves == 2 and self.VerticalMoves == True:
                self.XYZ.abs_move_mm('Z',-1*self.Z1Pos)
            if self.moveInc == 1 and self.TotalMoves == 4:
                #only move the Z for the 2/4 spot
                self.XYZ.abs_move_mm('Z',-1*self.Z2Pos)
            elif self.moveInc == 1 and self.TotalMoves == 2 and self.HorizontalMoves == True:
                self.XYZ.abs_move_mm('Y',-1*self.Y2Pos)
            elif self.moveInc == 1 and self.TotalMoves == 2 and self.VerticalMoves == True:
                self.XYZ.abs_move_mm('Z',-1*self.Z2Pos)
            if self.moveInc == 2 and self.TotalMoves == 4:
                #now only move Y so we are in the Y2, Z2 position
                self.XYZ.abs_move_mm('Y',-1*self.Y2Pos)
            else:
                print('We somehow are at the third loop but the total moves wasnt 4')
            if self.moveInc == 3 and self.TotalMoves == 4:
                #now only move Z so we are in the Y2, Z1 position
                self.XYZ.abs_move_mm('Z',-1*self.Z1Pos)
            self.currentY = round(self.XYZ.getPosition('Y'),2)
            self.currentZ = round(self.XYZ.getPosition('Z'),2)
            for deg in self.DegreeListDay1:
                ...
                try:
                    self.SMC.improved_move_abs(deg,int(20)) #the speed of the stage is usually set to 15 as my benchmark
                except:
                    self.NewErrorMessage = 'Stage did not successfully rotate to {} deg'.format(deg)
                    self.ExceptionThrown = True    
                    self.ExceptionIncrement = self.ExceptionIncrement + 1
                ...
                if self.HighMax == True:
                    ...
                    self.suffix = deg
                #Since we are just doing 2 images to stitch it is simplified from the looping structure
                self.realSuffix = 1
                while self.realSuffix <= self.NumberPerDeg:
                            try:
                                #self.Image=self.Camera.TakeImage(self.ExposureTime,self.FileLocation,(self.FileName+str(self.suffix)+'_'),(str(self.realSuffix)+'_'+str(self.Y1Pos)+'_mm'),self.ContrastRun,self.EMGain)
                                self.Image=self.Camera.TakeImage(self.ExposureTime,self.FileLocation,(self.FileName+str(self.suffix)+'_'),(str(self.realSuffix)+'_'+str(self.currentY)+'_mm'+str(self.currentZ)+'_mm'),self.ContrastRun,self.EMGain)
                                #timeBeforeSnap = time.time()
                                #this has been changed from before, now we have the degree within the "filename" and the number of the image as the suffix
                                #self.Image=self.Camera.TakeImage(self.ExposureTime,self.FileLocation,(self.FileName+str(self.CurrentYPos)+'_mm'+str(self.CurrentZPos)+'_mm'+str(self.suffix)+'_'),self.realSuffix,self.ContrastRun,self.EMGain)
                                #timeAfterSnap = time.time()-timeBeforeSnap
                                #print('Exp Time = {} actual time = {}'.format((self.ExposureTime/1000),timeAfterSnap))
                            except:
                                self.NewErrorMessage = 'Camera did not take picture properly, error in camera'
                                self.ExceptionThrown = True
                                self.ExceptionIncrement = self.ExceptionIncrement + 1
                            else:    
                                self.ContrastPath = self.FileLocation+'/ReSizedJPGs/'+self.FileName+str(self.suffix)+'_'+str(self.realSuffix)+'_'+str(self.currentY)+'_mm'+str(self.currentZ)+'_mm'+'_contrast.jpeg'
                                self.ContrastTiff = self.FileLocation+'/ReSizedJPGs/'+self.FileName+str(self.suffix)+'_'+str(self.realSuffix)+'_'+str(self.currentY)+'_mm'+str(self.currentZ)+'_mm'+'_contrast.tif'
                                self.JPGPath = self.FileLocation+'/ReSizedJPGs/'+self.FileName+str(self.suffix)+'_'+str(self.realSuffix)+'_'+str(self.currentY)+'_mm'+str(self.currentZ)+'_mm'+'.jpeg'
                                #self.ContrastPath = self.FileLocation+'/ReSizedJPGs/'+self.FileName+str(self.suffix)+'_'+str(self.realSuffix)+'_'+str(self.Y1Pos)+'_mm'+'_contrast'+'.jpeg'
                                #self.ContrastTiff = self.FileLocation+'/ReSizedJPGs/'+self.FileName+str(self.suffix)+'_'+str(self.realSuffix)+'_'+str(self.Y1Pos)+'_mm'+'_contrast.tif'
                                #self.JPGPath = self.FileLocation+'/ReSizedJPGs/'+self.FileName+str(self.suffix)+'_'+str(self.realSuffix)+'_'+str(self.Y1Pos)+'_mm'+'.jpeg'
                ...
                if self.ExceptionThrown == False:
                    ...
                    #send the remaining degrees to text if its the first time the error is sent
                    self.NewErrorMessage = 'Some error happened with the xyz stage most likely'
                    if self.ExceptionIncrement <= 1:
                        self.RemainingToText()
                    #make the optional frame red
                    self.OptionalFrame.grid_forget()
                    self.OptionalFrame = LabelFrame(self.TomographyFrame,text='Remaining Degrees Listed Here',bg='red',fg='white',padx=10,pady=10)
                    self.OptionalFrame.grid(row=1,column=2)
                    #make the general frame red
                    self.GeneralFrame.grid_forget()
                    self.GeneralFrame = LabelFrame(self.TomographyFrame, text='Error Error Error',bg='red',fg='white',padx=50,pady=100)
                    self.GeneralFrame.grid(row=1,column=0,columnspan=2)
                time.sleep(0.01)
                ...
                self.NewGenMessage = 'Expected time remaining {} min for the {} images left before next YZ position'.format(self.ExpectedTime,self.ImagesLeft)
                self.moveInc = self.moveInc + 1
    ...
    def UpdateGUI(self):
        if self.UpdateKill != True:
            ...
            if self.FirstPicture == True:
                root.after((int(self.ExposureTime)+8000), self.UpdateGUI)
            else:
            ...
            if self.HighMax == True:
                ...
                self.DegMes = self.RemainingList
    ...
    def Stop(self):
        ...
        if self.XYZUsed == True:
            try:
                doesntmatter=50
                self.XYZ = XYZMotion_Station.VXM(self.XYZPortEntry.get(),9600,0.1,doesntmatter)
            except:
                try:
    ...
    def TimeRemaining(self,ExposureTime,ImagesRemaining):
        ...
        MoveTime_s = 7.05*ImagesRemaining
        ImageTime_s = (ExposureTime/1000)*ImagesRemaining*2 #this has a *2 as there are actually two images per angle
    ...
    def ShowXYZ(self):
        ...
        self.XYZHideButton = Button(self.InitiateXYZ,text='Hide XYZ Control',padx=32,command=self.HideXYZ)
        ...
        self.XYZGoToSetup = Button(self.InitiateXYZ,text='Go To Setup Positions',padx=22,pady=5,command=self.SetupPositions)
        ...
        self.XYZHideButton.grid(row=0,column=2)
        ...
        self.ZSetEntry.insert(0,'16.0025')
        ...
        self.XMoveLabel = Label(self.ControlXYZ,text='X Movements: Range 0-126.35mm (4.974409in)')
        self.XZeroButton = Button(self.ControlXYZ,text='X Zero',command=lambda: self.ZeroStage('X'))
        self.YMoveLabel = Label(self.ControlXYZ,text='Y Movements: Range 0-127.365mm (5.01437in)')
        self.YZeroButton = Button(self.ControlXYZ,text='Y Zero',command=lambda: self.ZeroStage('Y'))
        self.ZMoveLabel = Label(self.ControlXYZ,text='Z Movements: Range 0-127.337mm (5.01328in)')
    ...
    def ExperimentalPositions(self):
        ...
        if self.XYZConnected == True:
            ...
            YExp = float(self.YSetEntry.get())*-1 #convention flip
            ZExp = float(self.ZSetEntry.get())*-1 #to correct for convention flip
    ...
    def SetupPositions(self):
            ...
            if self.XYZConnected == True:
                XSetup = float('0.01')
                YSetup = float('-0.01')
                ZSetup = float('-0.01')  #using -5.0 as I have z orientation flipped for convenience
    ...
    def XYZMoveRelative(self,Stage):
        if self.XYZConnected == True:
            if Stage == 'X':
                ...
                MoveDistance = float(self.YRelEntry.get())*-1       #mult by -1 for cenvtion flip
                if MoveUnits == 'mm':
    ...
    def XYZMoveAbsolute(self,Stage):
        if self.XYZConnected == True:
            if Stage == 'X':
                ...
                MoveTo = float(self.YAbsEntry.get())*-1
                if MoveUnits == 'mm':
